...
public class TestListCouriers {
    ...
    static void setUp() throws IOException {
        ...
        server.enqueue(TestUtil.createMockResponse().setBody(TestUtil.getJson("CourierList.json")));
        server.start();
    }
    ...
    public void testListCouriers() throws IOException, InterruptedException {
        ...
        Assertions.assertFalse(entity.hasError(),
                "No errors in response.");
        Assertions.assertNotNull(entity.getData(),
                "Response data cannot be empty.");
        Assertions.assertEquals(2, entity.getData().getTotal(),
                "Total mismatch.");
        Assertions.assertEquals("DHL", entity.getData().getCouriers().get(0).getName(),
                "The first couriers name was DHL.");
        ...
        assertEquals(HttpMethod.GET.getName(), recordedRequest.getMethod(),
                "HttpMethod is GET.");
        assertEquals(TestUtil.getRequestPath(EndpointPath.LIST_COURIERS), recordedRequest.getPath(),
